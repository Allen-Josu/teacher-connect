...
export default function TeacherInteractionPage() {
  ...
  const forums = [
    { id: 1, title: 'Teaching Strategies for Remote Learning', author: 'Dr. Sarah Johnson', replies: 24, likes: 45, time: '2h ago', category: 'Pedagogy', 
      content: 'I\'ve been experimenting with various remote learning strategies and wanted to share what\'s been working well in my classroom. Interactive breakout rooms have been particularly effective for group discussions.',
      responses: [
        { id: 1, author: 'Prof. Michael Chen', avatar: 'MC', text: 'Great insights! I\'ve also found that using collaborative tools like virtual whiteboards really helps engagement.', time: '1h ago', likes: 12 },
        { id: 2, author: 'Ms. Emily Rodriguez', avatar: 'ER', text: 'Have you tried using polls during lectures? I\'ve seen a significant increase in participation.', time: '45m ago', likes: 8 },
        { id: 3, author: 'Mr. David Kumar', avatar: 'DK', text: 'The breakout rooms are excellent! I also recommend recording sessions for students who need to review.', time: '30m ago', likes: 15 },
      ]
    },
    { id: 2, title: 'Best Practices for Student Engagement', author: 'Prof. Michael Chen', replies: 18, likes: 32, time: '5h ago', category: 'Discussion',
      content: 'Student engagement has been a challenge lately. I\'m looking for innovative ways to keep students motivated and participating actively in class discussions.',
      responses: [
        { id: 1, author: 'Dr. Sarah Johnson', avatar: 'SJ', text: 'Gamification works wonders! I use point systems and leaderboards to encourage participation.', time: '4h ago', likes: 10 },
        { id: 2, author: 'Dr. Lisa Anderson', avatar: 'LA', text: 'Real-world case studies make content more relatable. Students engage much better when they see practical applications.', time: '3h ago', likes: 14 },
      ]
    },
    { id: 3, title: 'Sharing Assessment Templates', author: 'Ms. Emily Rodriguez', replies: 12, likes: 28, time: '1d ago', category: 'Resources',
      content: 'I\'ve created some comprehensive assessment templates that have saved me tons of time. Would love to share them with the community and get feedback!',
      responses: [
        { id: 1, author: 'Mr. David Kumar', avatar: 'DK', text: 'This is incredibly helpful! Thank you for sharing. The rubrics are especially well-structured.', time: '20h ago', likes: 9 },
        { id: 2, author: 'Dr. Sarah Johnson', avatar: 'SJ', text: 'Love the clarity in your grading criteria. Mind if I adapt this for my math classes?', time: '18h ago', likes: 7 },
      ]
    },
    { id: 4, title: 'Interdisciplinary Project Ideas', author: 'Mr. David Kumar', replies: 31, likes: 56, time: '2d ago', category: 'Collaboration',
      content: 'I believe interdisciplinary projects create the most meaningful learning experiences. Let\'s brainstorm some collaborative project ideas across different subjects!',
      responses: [
        { id: 1, author: 'Prof. Michael Chen', avatar: 'MC', text: 'How about a science and art project? Students could create visual representations of scientific concepts.', time: '1d ago', likes: 18 },
        { id: 2, author: 'Ms. Emily Rodriguez', avatar: 'ER', text: 'I\'d love to collaborate on a chemistry and environmental science project!', time: '1d ago', likes: 13 },
        { id: 3, author: 'Dr. Lisa Anderson', avatar: 'LA', text: 'We could combine literature analysis with historical context. Students write from perspectives of historical figures.', time: '22h ago', likes: 11 },
      ]
    },
  ];
  ...
  const getAllMessages = (teacherId) => {
    const originalMessages = messages;
    const newMessages = chatMessages[teacherId] || [];
    return [...originalMessages, ...newMessages];
  };

  const toggleLikePost = (forumId) => {
    setLikedPosts(prev => ({
      ...prev,
  ...
  const toggleLikeReply = (forumId, replyId) => {
  ...
  const getAllReplies = (forum) => {
    const originalReplies = forum.responses || [];
    const newReplies = forumReplies[forum.id] || [];
    return [...originalReplies, ...newReplies];
  };
                  ...
                  {filteredTeachers.map((teacher) => (
                    <button
                      ...
                      className={`w-full p-4 flex items-start space-x-3 hover:bg-gray-50 transition-colors ${
                        selectedTeacher?.id === teacher.id ? 'bg-violet-50' : ''
                      }`}
                    >
                      <div className="relative">
                        ...
                        <span className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-white ${
                          teacher.status === 'online' ? 'bg-green-500' : teacher.status === 'away' ? 'bg-yellow-500' : 'bg-gray-400'
                        }`} />
                      </div>
                ...
                {selectedTeacher ? (
                  <>
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gray-50">
                      {getAllMessages(selectedTeacher.id).map((message) => (
                        <div
                          key={message.id}
              ...
              <div className="bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl">
                ...
                <div className="p-6 border-b border-gray-200">
                  <button
                    ...
                    className="flex items-center space-x-2 text-violet-600 hover:text-violet-700 mb-4 transition-colors"
                  >
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="flex items-center space-x-4 text-sm text-gray-600">
                    <span>Posted by <span className="font-medium">{selectedForum.author}</span></span>
                    <span>•</span>
                ...
                <div className="p-6 border-b border-gray-200 bg-gray-50">
                  <p className="text-gray-800 mb-4 leading-relaxed">{selectedForum.content}</p>
                  <div className="flex items-center space-x-6">
                    <button
                      onClick={() => toggleLikePost(selectedForum.id)}
                      className="flex items-center space-x-2 text-gray-600 hover:text-red-500 transition-colors"
                    >
                      <svg 
                        className={`w-5 h-5 transition-all ${likedPosts[selectedForum.id] ? 'fill-red-500 text-red-500' : 'fill-none'}`}
                        stroke="currentColor" 
                        viewBox="0 0 24 24"
                      >
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                      </svg>
                      <span className="font-medium">{selectedForum.likes + (likedPosts[selectedForum.id] ? 1 : 0)}</span>
                    </button>
                    <div className="flex items-center space-x-2 text-gray-600">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                      </svg>
                      <span className="font-medium">{selectedForum.responses.length + (forumReplies[selectedForum.id]?.length || 0)} replies</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-4">
                    {getAllReplies(selectedForum).map((reply) => (
                      <div key={reply.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:border-violet-200 transition-colors">
                        <div className="flex items-start space-x-3">
                          <div className={`w-10 h-10 ${reply.author === 'You' ? 'bg-gradient-to-br from-green-500 to-emerald-500' : 'bg-gradient-to-br from-violet-500 to-fuchsia-500'} rounded-full flex items-center justify-center text-white font-bold flex-shrink-0`}>
                            {reply.avatar}
                          </div>
                          <div className="flex-1">
                            <div className="flex items-center space-x-2 mb-2">
                              <span className={`font-semibold ${reply.author === 'You' ? 'text-green-600' : 'text-gray-900'}`}>{reply.author}</span>
                              <span className="text-sm text-gray-500">• {reply.time}</span>
                            </div>
                            <p className="text-gray-700 mb-3 leading-relaxed">{reply.text}</p>
                            <button
                              onClick={() => toggleLikeReply(selectedForum.id, reply.id)}
                              className="flex items-center space-x-2 text-gray-600 hover:text-red-500 transition-colors"
                            >
                              <svg 
                                className={`w-4 h-4 transition-all ${likedReplies[`${selectedForum.id}-${reply.id}`] ? 'fill-red-500 text-red-500' : 'fill-none'}`}
                                stroke="currentColor" 
                                viewBox="0 0 24 24"
                              >
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                              </svg>
                              <span className="text-sm font-medium">{reply.likes + (likedReplies[`${selectedForum.id}-${reply.id}`] ? 1 : 0)}</span>
                            </button>
                          </div>
                        </div>
                      </div>
                  ...
                  <div className="mt-6 border-t border-gray-200 pt-6">
                    ...
                    <div className="flex justify-end mt-3">
                      <button 
                        onClick={() => handleAddReply(selectedForum.id)}
                        className="px-6 py-2 bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white rounded-lg hover:shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
